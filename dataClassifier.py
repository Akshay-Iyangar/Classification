...
def enhancedFeatureExtractorDigit(datum):
    ...
    #create a mask of 2d array of white spaces
    counter_region1=0
    counter_region2=0
    counter_region3=0
    counter_region4=0
    mask = [[0 for j in range(DIGIT_DATUM_HEIGHT)] for i in range(DIGIT_DATUM_WIDTH)]
            
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if((y>=8 and y<=13) and (x>=8 and x<=13)):
                if(mask[x][y]==0 and datum.getPixel(x,y)==0):
                    counter_region1=counter_region1+1
            if((y>=8 and y<=13) and (x>=13 and x<=19)):
                if(mask[x][y]==0 and datum.getPixel(x,y)==0):
                    counter_region2=counter_region2+1
            if((y>=8 and y<=13) and (x>=8 and x<=13)):
                if(mask[x][y]==0 and datum.getPixel(x,y)==0):
                    counter_region3=counter_region3+1
            if((y>=8 and y<=13) and (x>=13 and x<=19)):
                if(mask[x][y]==0 and datum.getPixel(x,y)==0):
                    counter_region4=counter_region4+1

    features[1]=0
    features[2]=0
    features[3]=0
    features[4]=0 
    if(counter_region1>19):
        features[1]=1
    if(counter_region2>19):
        features[2]=1
    if(counter_region3>19):
        features[3]=1
    if(counter_region4>19):
        features[4]=1 
